...
class AboutController extends Controller
{
        ...
        if ($user->detail_id == 0) {
            ...
            if ($validator->fails()) {
                ...
                if ($request->hasFile('photo')) {
                    ...
                    $destination_photo = '/student/pas_photo_' . time() . '.' .$file_ext_photo;
                    Storage::disk('s3')->put($destination_photo, file_get_contents($request->file('photo')));
                    $data[0]['name'] = $request->file('photo')->getClientOriginalName();
                    $data[0]['path'] = $destination_photo;
                    $data[0]['status'] = 'pasphoto';
                    $data[0]['mime'] = $file_ext_photo;

                    $student = Student::find($student->id);
                    $student->thumbnail_image_path = $destination_photo;
                    $student->save();
                }

                if ($request->hasFile('akta')) {
                    $file_ext_akta = $request->file('akta')->getClientOriginalExtension();
                    $destination_akta = '/student/akta_' . time() . '.' .$file_ext_akta;
                    Storage::disk('s3')->put($destination_akta, file_get_contents($request->file('akta')));
                    $data[1]['name'] = $request->file('akta')->getClientOriginalName();
                    $data[1]['path'] = $destination_akta;
                ...
                if ($request->hasFile('kartu_keluarga')) {
                    ...
                    $destination_kk = '/student/kartu_keluarga_' . time() . '.' .$file_ext_kk;
                    Storage::disk('s3')->put($destination_kk, file_get_contents($request->file('kartu_keluarga')));
                    $data[2]['name'] = $request->file('kartu_keluarga')->getClientOriginalName();
                    $data[2]['path'] = $destination_kk;
                ...
                if ($request->hasFile('ijasah')) {
                    ...
                    $destination_ijazah = '/student/ijasah_' . time() . '.' .$file_ext_ijazah;
                    Storage::disk('s3')->put($destination_ijazah, file_get_contents($request->file('ijasah')));
                    $data[3]['name'] = $request->file('ijasah')->getClientOriginalName();
                    $data[3]['path'] = $destination_ijazah;
            ...
            if ($request->hasFile('photo')) {
                ...
                if (!empty($sa)) {
                    if (!empty($sa->path) && Storage::disk('s3')->exists($sa->path)) {
                        Storage::disk('s3')->delete($sa->path);
                    }
                    
                    $sa->name = $file_name_photo;
                    $sa->path = $destination_photo;
                    $sa->status = 'pasphoto';
    ...
    public function __update_profile(Request $request)
    {
